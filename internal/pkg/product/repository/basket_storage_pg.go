...
func (p *ProductStorage) selectOrdersInBasketByUserID(ctx context.Context, tx pgx.Tx,
	userID uint64) ([]*models.OrderInBasket, error) {
	var orders []*models.OrderInBasket

	SQLSelectOrdersInBasketByUserID :=
		`SELECT  "order".id, "order".owner_id, "order".product_id, 
	...
	_, err = pgx.ForEachRow(ordersInBasketRows, []any{
		...
		orders = append(orders, &models.OrderInBasket{
			ID:           curOrder.ID,
			OwnerID:      curOrder.OwnerID,
			ProductID:    curOrder.ProductID,
...
func (p *ProductStorage) updateOrderCountByOrderID(ctx context.Context, tx pgx.Tx, orderID uint64,
	newCount uint32) error {
	SQLUpdateOrderCountByOrderID :=
		`UPDATE public."order"
		 SET count=$1
		 WHERE id=$2`
...
func (p *ProductStorage) getOrderByID(ctx context.Context, tx pgx.Tx, orderID uint64) (*models.Order, error) {
	SQLGetOrderByID :=
		`SELECT owner_id, product_id, count, status, created_at, updated_at, closed_at 
...
func (p *ProductStorage) updateOrderStatusByOrderID(ctx context.Context, tx pgx.Tx, orderID uint64,
	newStatus uint8) error {
	SQLUpdateOrderCountByOrderID :=
		`UPDATE public."order"
		 SET status=$1
		 WHERE id=$2`
...
func (p *ProductStorage) decreaseAvailableCountByOrderID(ctx context.Context, tx pgx.Tx, orderID uint64) error {
	SQLDecreaseAvailableCountByOrderID :=
...
func (p *ProductStorage) UpdateOrderStatus(ctx context.Context, orderID uint64, newStatus uint8) (*models.Order, error) {
	updatedOrder := &models.Order{} //nolint:exhaustruct
	...
	err := pgx.BeginFunc(ctx, p.pool, func(tx pgx.Tx) error {
		...
		if newStatus <= curStatus {
			fmt.Errorf(myerrors.ErrTemplate, ErrLessStatus)
		}
