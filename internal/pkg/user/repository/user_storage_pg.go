...
func (u *UserStorage) getUserFavourites(ctx context.Context, tx pgx.Tx,
	userID uint64) ([]*models.ProductInFeed, error) {

	SQLGetUserFavourites :=
		`SELECT p.id, p.title
		FROM public."product" p
		JOIN public."favourites" f ON p.id = f.product_id
		WHERE f.owner_id = $1`

	productsInFavouritesRows, err := tx.Query(ctx, SQLGetUserFavourites, userID)
	if err != nil {
		u.logger.Errorln(err)
...
func (u *UserStorage) GetUserFavourites(ctx context.Context, userID uint64) ([]*models.ProductInFeed, error) {
